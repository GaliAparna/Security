...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public Employee add(Employee emp) throws EmployeeException {
		...
		if(emp!=null) {
			if(empRepo.existsById(emp.getEmpId())) {
				throw new EmployeeException("Employee exsits with given Id");
				
			}
			else if(empRepo.existsByMobileNumber(emp.getMobileNumber())) {
				throw new EmployeeException("Employee exsits with given mobileNumber");
				...
				throw new EmployeeException("Employee exsits with given email");
	...
	public Employee save(Employee emp) throws EmployeeException {
		...
		if(emp!=null) {
			...
			if(oldEmp==null) {
				throw new EmployeeException("There doesnt exists an employee with that id");
			}else if(!oldEmp.getMobileNumber().equals(emp.getMobileNumber()) && empRepo.existsByMobileNumber(emp.getMobileNumber())) {
				throw new EmployeeException("There doesnt exists an employee with that mobile number");
			}else if(!oldEmp.getEmail().equals(emp.getEmail()) && empRepo.existsByEmail(emp.getEmail())) {
				throw new EmployeeException("There doesnt exists an employee with that email");
			}else {
				emp=empRepo.save(emp);
			}
	...
	@Override
	public List<Employee> findAllBasicRange(double lb, double ub) {
